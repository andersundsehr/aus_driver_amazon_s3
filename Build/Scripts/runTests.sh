...
case ${TEST_SUITE} in
    ...
    functional)
        ...
        # TODO does this work with podman?

        ${CONTAINER_BIN} run --rm ${CI_PARAMS} -d --name minio-func-${SUFFIX} --network ${NETWORK} -w /minio-data -v ${ROOT_DIR}:/app --entrypoint= --tmpfs /minio-data/:rw,noexec,nosuid minio/minio:RELEASE.2024-09-22T00-33-43Z sh /app/Build/testing-docker/prepare-minio-data.sh
        # TODO wie warten wir besser? die sachen sollen erst testen wenn der minio mit dem "import" durch ist.
        sleep 10
        waitFor minio-func-${SUFFIX} 9000
        ...
        case ${DBMS} in
            mariadb)
                ...
                CONTAINERPARAMS="-e minioHost=minio-func-${SUFFIX} -e typo3DatabaseDriver=${DATABASE_DRIVER} -e typo3DatabaseName=func_test -e typo3DatabaseUsername=root -e typo3DatabaseHost=mariadb-func-${SUFFIX} -e typo3DatabasePassword=funcp"
            ...
            mysql)
                ...
                CONTAINERPARAMS="-e minioHost=minio-func-${SUFFIX} -e typo3DatabaseDriver=${DATABASE_DRIVER} -e typo3DatabaseName=func_test -e typo3DatabaseUsername=root -e typo3DatabaseHost=mysql-func-${SUFFIX} -e typo3DatabasePassword=funcp"
                ${CONTAINER_BIN} run ${CONTAINER_COMMON_PARAMS} --name functional-${SUFFIX} ${XDEBUG_MODE} -e XDEBUG_CONFIG="${XDEBUG_CONFIG}" ${CONTAINERPARAMS} ${IMAGE_PHP} "${COMMAND[@]}"
                SUITE_EXIT_CODE=$?
                ;;
            postgres)
                ...
                CONTAINERPARAMS="-e minioHost=minio-func-${SUFFIX} -e typo3DatabaseDriver=pdo_pgsql -e typo3DatabaseName=bamboo -e typo3DatabaseUsername=funcu -e typo3DatabaseHost=postgres-func-${SUFFIX} -e typo3DatabasePassword=funcp"
                ${CONTAINER_BIN} run ${CONTAINER_COMMON_PARAMS} --name functional-${SUFFIX} ${XDEBUG_MODE} -e XDEBUG_CONFIG="${XDEBUG_CONFIG}" ${CONTAINERPARAMS} ${IMAGE_PHP} "${COMMAND[@]}"
                SUITE_EXIT_CODE=$?
                ;;
            sqlite)
                ...
                CONTAINERPARAMS="-e minioHost=minio-func-${SUFFIX} -e typo3DatabaseDriver=pdo_sqlite --tmpfs ${ROOT_DIR}/.Build/web/typo3temp/var/tests/functional-sqlite-dbs/:rw,noexec,nosuid"
                ${CONTAINER_BIN} run ${CONTAINER_COMMON_PARAMS} --name functional-${SUFFIX} ${XDEBUG_MODE} -e XDEBUG_CONFIG="${XDEBUG_CONFIG}" ${CONTAINERPARAMS} ${IMAGE_PHP} "${COMMAND[@]}"
                SUITE_EXIT_CODE=$?
                ;;
        esac
